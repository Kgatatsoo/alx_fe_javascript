...
function handleSync(serverQuotes) {
  ...
  if (updatesMade) {
    ...
    alert("Quotes synced with server!");  // Added alert for successful sync
  }
}
...
function addQuote() {
  ...
  if (newQuoteText && newQuoteCategory) {
    ...
    // Send the new quote to the server using POST request
...
async function addQuoteToServer(newQuote) {
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify(newQuote),  // Stringify the new quote to send as JSON
    });
    ...
    // Optionally, show notification or handle success
    showNotification('Quote added to the server successfully!');
  } catch (error) {
    console.error('Error adding quote to server:', error);
    showNotification('Failed to add quote to the server.');
  }
}
